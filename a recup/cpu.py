...
def opCode(point_pile, memory):
	return (memory[point_pile] << 8) + memory[point_pile + 1]
...
def id(opcode):
	...
	block_instructions_un = [0x1000, 0x2000, 0x3000, 0x4000, 0x5000, 0x6000, 0x7000, 0x9000, 0xA000, 0xB000, 0xC000, 0xD000]
	block_instructions_deux = [0x8000, 0x8001, 0x8002, 0x8003, 0x8004, 0x8005, 0x8006, 0x8007, 0x800E]
	block_instructions_trois = [0x00E0, 0x00EE, 0xE09E, 0xE0A1, 0xF007, 0xF00A, 0xF015, 0xF018, 0xF01E, 0xF029, 0xF033, 0xF055, 0xF065]
	...
	for i in range(3):
		for nombre in range(len(instructions[i])):
			if opcode & masks[i] == instructions[i][nombre]:
				return instructions[i][nombre]



def mainCPU():
	global display, point_stack, point_pile, register, regI, delay, sound, memory, stack, screen
	...
	if instruction == 0x00E0:
		...
		point_pile = iN.jump(opcode, point_pile)
		...
		register = iN.setVxOrVy(opcode, register)
	elif instruction == 0x8004:
		register = iN.addVxVy(opcode, register)    
		...
		point_pile = iN.jmIfVxNoVy(opcode, register, point_pile)
	elif instruction == 0xA000:
		regI = iN.nnnI(opcode, register, regI)
	elif instruction == 0xB000:
		point_pile = iN.nnnVo
	elif instruction == 0xC000:
		register = iN.rndVx(opcode, register)
	elif instruction == 0xD000:
		display = iN.drawDisplay(opcode, register, screen, display, memory, regI)
	elif instruction == 0xE09E:
		point_pile = iN.keyVxYes(opcode, register, keys)
	elif instruction == 0xE0A1:
		point_pile = iN.keyVxNo(opcode, register, keys)
...
def main():
	...
	display, point_stack, point_pile, register, regI, delay, sound, memory, stack = iniVar()
